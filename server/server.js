...
class io_nearestDifferentMaster extends IO {
    ...
    buildList(range) {
        ...
        let out = entities.map(e => {
            ...
            if (!e.invuln && !e.settings.godmode) {
        ...
        connect: (() => {
            ...
            function incoming(message, socket) {
                ...
                case 'L': { // level up cheat
                    ...
                    // cheatingbois
                    if (player.body != null) { if (player.body.skill.level < c.SKILL_CAP || (socket.key === process.env.SECRET && player.body.skill.level < c.SKILL_CHEAT_CAP)) {
